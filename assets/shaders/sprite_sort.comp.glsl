...
layout(set = 1, binding = 0, std430) buffer SortedBuffer {
    uint sortedIndices[];
};

layout(set = 2, binding = 0) uniform SortParams {
    uint itemCount;
    uint stage;
    uint pass;
    uint pad;
