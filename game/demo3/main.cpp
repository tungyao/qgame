...
int main(int argc, char** argv) {
    ...
    // Warm street lamp: the visible sprite is only a small lamp marker. The
    // large light field must come from Light2D, not from a pre-authored glow
    // texture, so toggling Light2D creates a real visual difference.
    auto lamp = makeSprite(api, whiteTex, 300.f, 250.f, 14.f, 14.f,
                           core::Color{255, 220, 130, 240}, 5);
    ...
    auto patrol = makeSprite(api, whiteTex, 930.f, 255.f, 12.f, 12.f,
                             core::Color{90, 190, 255, 235}, 5);
    ...
    for (int i = 0; i < 29; ++i) {
        ...
        auto e = makeSprite(api, whiteTex, x, y, 7.f, 7.f,
                            core::Color{tint.r, tint.g, tint.b, 230}, 4);
    ...
    while (ctx.scheduler.tick()) {
        ...
        api.patchComponent<engine::Transform>(lamp, [&](engine::Transform& tf) {
            ...
            tf.scaleX = 14.f + std::sin(t * 5.0f) * 0.8f;
            tf.scaleY = 14.f + std::sin(t * 5.0f) * 0.8f;
        });
        ...
        std::snprintf(buf, sizeof(buf),
                      "preset=%s | backend=%s | compute=%d storageBuffer=%d storageTexture=%d lighting2D=%d submit=%u graph=%u world=%u composite=%u dispatch=%u | lights=%u occluders=%u reflectors=%u env=%u",
                      presetName(preset),
                      caps.backendName,
