...
int main(int argc, char** argv) {
    ...
    cfg.windowTitle = "QGame Demo3 - Vulkan 2D Lighting Data";
    ...
    // These translucent black quads are handwritten shadow previews for demo3.
    // They make the intended result visible before the Vulkan compute pass
    // exists, but no engine logic should depend on them.
    auto shadowA = makeSprite(api, whiteTex, 540.f, 430.f, 270.f, 140.f,
                              core::Color{0, 0, 0, 100}, 4);
    api.patchComponent<engine::Transform>(shadowA, [](engine::Transform& tf) {
        tf.rotation = 0.32f;
    });
    auto shadowB = makeSprite(api, whiteTex, 730.f, 395.f, 230.f, 100.f,
                              core::Color{0, 0, 0, 82}, 4);
    api.patchComponent<engine::Transform>(shadowB, [](engine::Transform& tf) {
        tf.rotation = -0.24f;
    });
    ...
    auto title = makeText(api, font, "Demo3: 2D Lighting Data Path (L0/L1)", 28.f, 34.f, 24.f,
                          core::Color{235, 242, 255, 255});
    (void)title;
    makeText(api, font,
             "Debug glow/shadow quads preview the future compute pass.",
             28.f, 66.f, 14.f, core::Color{170, 188, 210, 255});
    makeText(api, font,
             "Real test data: Light2D + LightOccluder2D + Reflector2D + Environment2D.",
    ...
    while (ctx.scheduler.tick()) {
        ...
        std::snprintf(buf, sizeof(buf),
                      "backend=%s | compute=%d storageBuffer=%d storageTexture=%d lighting2D=%d | lights=%u occluders=%u reflectors=%u env=%u",
                      caps.backendName,
